...
function Season_Suggestions() {
    const navigate = useNavigate();
    const token = sessionStorage.getItem('token')
    const {All_Seasons} = Appstate();
    ...
    const GetColors = async(id)=>{
        try{
            const verifyData = await axios.get(`${Url}/SuggestBySeason/${id}`,{
                headers:{
            ...
            setMoodColors(verifyData.data.find_SeasonColor)
        }
        catch(err){
            // console.log(err)
  ...
  return (
    <BaseApp PageTitle="Suggestions By Season">
                    ...
                    <form onSubmit={handleSubmit}>
                        <FormControl sx={{ m: 1, minWidth: 250 }}>
                            ...
                            <Select
                              ...
                              onBlur={handleBlur}
                            >
                              {All_Seasons.map((dt,idx)=>(
                                <MenuItem value={dt} key={idx}>{dt}</MenuItem>
                              ))}
                            </Select>
                            <FormHelperText>Select a Season</FormHelperText>
                        </FormControl>
...
export default Season_Suggestions
...
function Admin_UpdateDailyColors() {
    const navigate = useNavigate();
    const {id} = useParams();
    const token = sessionStorage.getItem('token')
    const {Dates,dailyColors} = Appstate();
    const editColor = dailyColors.filter((dal)=>dal._id==id)
    ...
    const AddColor = async(data)=>{
        try{
            const verifyData = await axios.put(`${Url}/Admin/UpdateDailyColor/${id}`,data,{
                headers:{
  ...
  return (
    <Admin_BaseApp PageTitle="Update Datewise Color">
...
export default Admin_UpdateDailyColors
...
function Admin_UpdateColor() {
    const navigate = useNavigate();
    const {id} = useParams();
    const token = sessionStorage.getItem('token')
    const {All_Moods,All_Seasons,All_Weather,All_Occasions,colors} = Appstate();
    const editColor = colors.filter((dat)=>dat._id==id)

    // Form Validation
    const{values,handleChange,handleBlur,handleSubmit,errors,touched} = useFormik({
        initialValues:{
            Color_Name :editColor[0].Color_Name,
            Color_Look :editColor[0].Color_Look,
            Fact :editColor[0].Fact,
            Image :editColor[0].Image,
            Mood :editColor[0].Mood,
            Season :editColor[0].Season,
            Weather :editColor[0].Weather,
            Occasions :editColor[0].Occasions
        },
        ...
        const UpdateColor = async(data)=>{
            try{
                const verifyData = await axios.put(`${Url}/Admin/UpdateColor/${id}`,data,{
                    headers:{
  ...
  return (
    <Admin_BaseApp PageTitle="Update Color">
            ...
            <div className='AdminAdd_LoginCont'>
                <div className='AdminAdd_FormCont'>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <br/>
                        <Button size="lg" type='submit'>⬆️Update</Button>
                    </form>
                </div>
            </div>
        </div>
...
export default Admin_UpdateColor
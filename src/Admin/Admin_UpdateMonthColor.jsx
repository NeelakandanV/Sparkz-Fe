...
function Admin_UpdateMonthColor() {
  const navigate = useNavigate();
  const {id} = useParams();
  const token = sessionStorage.getItem('token')
  const {Months, monthColors} = Appstate();
  const editColor = monthColors.filter((dal)=>dal._id==id)
  ...
  const AddColor = async(data)=>{
      try{
          const verifyData = await axios.put(`${Url}/Admin/UpdateMonthColor/${id}`,data,{
              headers:{
  ...
  return (
    <Admin_BaseApp PageTitle="Update Monthwise Color">
        <div className="Crebtn">
            <Button size="lg" onClick={()=>navigate("/Admin/MonthColor")}>⬅️ Monthwise Colors</Button>
        </div>
            ...
            <div className='AdminAdd_LoginCont'>
                <div className='AdminAdd_FormCont'>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <br/>
                        <Button size="lg" type='submit'>⬆️ Update</Button>
                    </form>
                </div>
            </div>
        </div>       
...
export default Admin_UpdateMonthColor